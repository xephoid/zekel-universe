...
- Neither Guts nor Gears has its own canvas in `docs/design/neither-guts-nor-gears/` (start with its README) and its own routing document, `docs/design/SCREEN-ROUTING.md`. Its table is a game-drawn screen (`apps/web/src/glue/ngg/`); `/dev/ngg` on the dev server draws any captured engine view.
